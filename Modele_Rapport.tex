...
\section{Présentation du problème}
Il existe déjà beaucoup d'études concernant des problèmes de plus court chemin avec l'objectif unique en utilisant \textsc{Dijkstra Algorithme} \cite{1}. L'objectif est de trouver un chemin en minimisant la somme du coût de ses arcs entre le départ et la destination. Mais l'optimisation d'un objectif unique n'est pas suffisante, car il existe plusieurs objectifs que nous devons considérer dans le problème véritable.
...
Selon la table ci-dessus, nous ne pouvons pas déterminer laquelle est la meilleure. Ca dépend la sélection de l'utilisateur. Ce type de problème est appelé \textsc{Multiobjective problem} \cite{2}. 
...
Dans cette partie, je vais détailler les différentes méthodes utilisées comprenant \textsc{Dijkstra Algorithme}, la solution de l'optimisation multi-objectif.
...
\textbf{Principe de l'algorithme de Dijkstra}
...
\textbf{Exemple de l'algorithme de Dijkstra}
\begin{figure}[!ht]
...
\subsubsection{Algorithme de BCA*}
...
\textbf{Principe de l'algorithme de BCA*}

\textbf{Exemple de l'algorithme de BCA*} 

\begin{figure}[!ht]
\label{fig:Un graphe à traiter}
\centering
\includegraphics[width=10cm]{Schemas/traiter.pdf}
...
\centering
\includegraphics[width=10cm]{Schemas/solution.pdf}
